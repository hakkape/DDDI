...
class BuildNetwork(object):
    ...
    def __init__(self, problem_data):
        ...
        for a, destinations in problem_data.network.items():
            for b, transit_time in destinations.items():
                self.network.add_edge(a, b, weight=transit_time, capacity=problem_data.capacities.get((a,b), 1.0), fixed_cost=problem_data.fixed_cost.get((a,b), transit_time), var_cost=problem_data.var_cost.get((a,b), 0))
    ...
    def create_arcs(self, intervals, paths=None):
        ...
        arcs = {k: tuplelist(self.lower_bound_arcs(c, intervals, cache, redirected_arcs)) if not paths or paths == None else self.fixed_path_arcs(paths[k], c, intervals)
    ...
    def fixed_path_arcs(self, path, commodity, intervals):
        ...
        for n in intervals:
            for e in edges.select(n[0], '*'):
                ...
                if arc != None and arc[1] != None:  # validity check is done in find_best_interval
                    arcs.append(arc)
    ...
    def is_arc_valid2(self, arc, origin, dest, origin_to_arc, arc_to_arc, arc_to_dest):
        return not ((arc[0] == None or arc[1] == None) or                                   # is valid node
                    (origin_to_arc == None or arc_to_dest == None) or                       # invalid path
    ...
    def iterate_flow(self, sorted_intervals, node):
        ...
        #while next != None:
        #    arc = (next, (intervals.select(node, next[2], '*') or [None])[0])  # gets first item or None
        #    next = arc[1]
        #    yield arc       # at end returns (last, None)

    #def find_upper_interval(self, nodes, node, time):
    #    for n in nodes.select(node, '*', '*'):
    #        if time == n[1]:
    #            return n

    #        if time > n[1] and time < n[2]:
    #            next = nodes.select(node, n[2], '*')
    #            return next[0] if any(next) else None  # return next node

    #    return None

    #def upper_bound_arcs(self, commodity, nodes):
    #    arcs = tuplelist()
    ...
    #                n2 = self.find_upper_interval(nodes, e[1], n[2] + self.network[n[0]][e[1]]['weight'])
    ...
    def second_shortest_path_network(self):
        ...
        for c in self.commodities:
            ...
            path = second_shortest_path(self.network, c['a'][0], c['b'][0], path=path)
            t = c['a'][1]
    ...
    def all_second_shortest_path_network(self):
        ...
        second_shortest_paths = {a: {b: second_shortest_path(self.network, a,b, shortest_paths[a][b]) 
        ...
        for n in self.network.nodes():
            for c in self.commodities:
                if n in shortest_paths[c['a'][0]] and c['b'][0] in shortest_paths[n]:
                    ...
                    # second shortest path
                    if second_shortest_paths[c['a'][0]][n] != None and second_shortest_paths[n][c['b'][0]] != None:
                        earliest_to_n = c['a'][1] + second_shortest_paths[c['a'][0]][n]
                        latest_from_n = c['b'][1] - second_shortest_paths[n][c['b'][0]]
