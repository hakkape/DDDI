...
algorithm_option = enum(default=0, multiplex=1, eclectic=2, reduced=3, adaptive=4)
...
class IntervalSolver(object):
    ...
    def __init__(self, problem, time_points=None, full_solve=True, fixed_paths=[], suppress_output=False, gap=MIP_GAP, algorithm=ALGORITHM, full_discretization=False, full_results_log=None, environment=None):
        ...
        self.environment = environment if environment != None else Env("")
        model = self.model = Solver(GUROBI, quiet=(self.fixed_timepoints_model == False or suppress_output == True), env=self.environment)
        
        if model.solver == CPLEX and full_results_log != None:
            self.model.model.set_results_stream(full_results_log)
        ...
        if full_discretization == False:
            ...
            # set which interval contains the origin/destination
            self.origin_destination = {k: ( (c['a'][0], ceil(c['a'][1]), ceil(c['a'][1])+1), (c['b'][0], floor(c['b'][1]), floor(c['b'][1])+1)) for k,c in enumerate(self.commodities)}
            self.intervals = tuplelist(self.cons_network.nodes())
        ...
        if PATH_CUTS:
            ...
            for k,G in enumerate(self.timed_network):
                ...
                for n in self.network.nodes():
                    outflow = [d['x']*self.transit(a1[0],a2[0]) for a1,a2,d in G.edges(data=True) if a1[0] == n and a2[0] != n and 'x' in d]
    ...
    def user_cuts(self):
        ...
        if USER_CUTS:
            for a1,a2,d in self.cons_network.out_edges(data=True):
                if 'z' in d and d['z'] != None:
                    for c in d['K']:
                        self.constraints_user.append(self.model.addConstr(d['z'] >= math.ceil(self.commodities[c]['q']/self.network[a1[0]][a2[0]]['capacity']) * self.timed_network[c][a1][a2]['x'], 'user'))
                ...
                for i in self.intervals.select(orig):
                    for k in K:
                        ...
                        for a1,a2 in self.timed_network[k].out_edges(i, data=False):
                            d = self.cons_network[a1][a2]
                            if 'z' in d and d['z'] != None:
                                if not dispatch_arcs:
                                    max_capacity = self.network[a1[0]][a2[0]]['capacity']
                                else:
                                    if max_capacity < self.network[a1[0]][a2[0]]['capacity']:
                                        max_capacity = self.network[a1[0]][a2[0]]['capacity']

                                dispatch_arcs.add((a1,a2))

                cons = self.model.addConstr(quicksum(self.cons_network[a1][a2]['z'] for a1,a2 in dispatch_arcs) >= math.ceil(q/max_capacity), 'user')
    ...
    def build_network(self):
        ...
        for k in range(len(self.commodities)):
            ...
            # Create timed arcs
            all_arcs.append(list(filter(v, (((e[0], self.S, self.T), (e[1], self.S, self.T)) 
                        for e in (self.network.edges() if not self.fixed_paths else self.fixed_paths[k])))))

            G.add_edges_from(((a[0], a[1], {
                'x': self.model.addVar(obj=(self.problem.var_cost[k].get((a[0][0],a[1][0]),0) * self.commodities[k]['q'] if a[0][0] != a[1][0] else self.problem.var_cost[k].get((a[0][0],a[1][0]),0) * self.commodities[k]['q'] * (a[0][2] - a[0][1])), lb=0, ub=1, type=self.model.binary() if ALLOW_SPLIT == False else self.model.continuous(), name='x' + str(k) + ',' + str(a)),
                'y': self.model.addVar(obj=(0 if a[0][0] != a[1][0] else -self.problem.var_cost[k].get((a[0][0],a[1][0]),0) * self.commodities[k]['q'] * (a[0][2] - a[0][1])), lb=0, ub=1, type=self.model.binary() if ALLOW_SPLIT == False else self.model.continuous(), name='y' + str(k) + ',' + str(a)),
               })  for a in all_arcs[k]))
        ...
        def create_data(a,coll):
            ...
            return {'z': self.model.addVar(obj=(self.network[a[0][0]][a[1][0]]['fixed_cost']), lb=0,
#                                                                             ub=ceil(sum(self.commodities[c]['q'] for c in K)),
                                                                             ub=self.model.inf(), 
...
#        self.cons_network.add_edges_from(((a[0],a[1],{'z': self.model.addVar(obj=(self.network[a[0][0]][a[1][0]]['fixed_cost']), lb=0,
#                                                                             ub=ceil(sum(self.commodities[c]['q'] for c in set(map(itemgetter(1),K)))),
##                                                                             ub=self.model.inf(), 
#                                                                             name='z' + str(a), type=self.model.integer()), 
...
#                                            for a,K in cons))
    ...
    def build_full_network(self):
        ...
        time_points = [(n,t) for n in self.network.nodes() for t in range(int(self.S), int(self.T+1), 1)]
        interval_cache = {g: [(g,t1,t2) for t1,t2 in pairwise(sorted(set(map(itemgetter(1), coll)) | set([self.S, self.T])))] for g,coll in itertools.groupby(time_points, itemgetter(0)) }
        ...
        for k,c in enumerate(self.commodities):
            ...
            arcs.append([arc for n in self.network.nodes() if n in origin_to_arc and self.shortest_path(k, n, dest[0]) != None
                                    for arc in pairwise(interval_cache[n]) if self.is_valid_storage_arc(arc, origin, dest, origin_to_arc.get(n, None), self.shortest_path(k, n, dest[0]))])

            for e in self.network.edges():
                origin_to_arc = self.shortest_path(k, origin[0], e[0])
                arc_to_dest = self.shortest_path(k, e[1], dest[0])

                if origin_to_arc != None and arc_to_dest != None:
        ...
        for k,c in enumerate(self.commodities):
            ...
            G.add_edges_from(((a[0], a[1], {'x': self.model.addVar(obj=(self.problem.var_cost[k].get((a[0][0],a[1][0]),0) * self.commodities[k]['q'] * ((a[0][2] - a[0][1]) if a[0][0] == a[1][0] else 1)), lb=0, ub=1, type=self.model.binary() if ALLOW_SPLIT == False else self.model.continuous(), name='x' + str(k) + ',' + str(a)) })
                                for a in arcs[k]))
            self.timed_network.append(G)
        ...
        cons = itertools.groupby(sorted((a,k) for k,k_arcs in enumerate(arcs) for a in k_arcs if a[0][0] != a[1][0]), itemgetter(0))
        self.cons_network.add_edges_from(((a[0],a[1],{'z': self.model.addVar(obj=(self.network[a[0][0]][a[1][0]]['fixed_cost']), lb=0, ub=self.model.inf(), name='z' + str(a), type=self.model.integer()), 
                                                        'K': set(map(itemgetter(1),K))}) 
                                            for a,K in cons))
        return arcs
    ...
    def create_arcs_between_nodes(self, k, n1, n2, origin, dest, origin_intervals, destination_intervals, redirected_arcs):
        ...
        for i1 in origin_intervals:
            ...
            if time < i2[2] and (i1 != None and i2 != None and (i2[0] != origin[0] or i1[0] == origin[0]) and ((i1[0] != dest[0] or i2[0] == dest[0]) and (origin[1] + origin_to_arc < i1[2]) and (i2[1] + arc_to_dest <= dest[1]) and (((dest[1] - arc_to_dest >= origin[1] + origin_to_arc + arc_to_arc) and (origin[1] + origin_to_arc + arc_to_arc < i2[2]) and (i1[1] + arc_to_arc + arc_to_dest <= dest[1]))))):
                ...
                while i2 != None and i1[2] + transit_time >= i2[2]:
                    if (i1 != None and i2 != None and (i2[0] != origin[0] or i1[0] == origin[0]) and ((i1[0] != dest[0] or i2[0] == dest[0]) and (origin[1] + origin_to_arc < i1[2]) and (i2[1] + arc_to_dest <= dest[1]) and (((dest[1] - arc_to_dest >= origin[1] + origin_to_arc + arc_to_arc) and (origin[1] + origin_to_arc + arc_to_arc < i2[2]) and (i1[1] + arc_to_arc + arc_to_dest <= dest[1]))))):
                        new_arcs.append((i1,i2))
                ...
                if i1[2] + transit_time < i2[2] and (i1 != None and i2 != None and (i2[0] != origin[0] or i1[0] == origin[0]) and ((i1[0] != dest[0] or i2[0] == dest[0]) and (origin[1] + origin_to_arc < i1[2]) and (i2[1] + arc_to_dest <= dest[1]) and (((dest[1] - arc_to_dest >= origin[1] + origin_to_arc + arc_to_arc) and (origin[1] + origin_to_arc + arc_to_arc < i2[2]) and (i1[1] + arc_to_arc + arc_to_dest <= dest[1]))))):
                    new_arcs.append((i1,i2))
    ...
    def walks(self, G, t, top=False):
        roots = []
        stack = [([t],[])] # tail nodes, current path
        ...
        while stack:
            ...
            for v in tails:
                ...
                while edges:  # choose latest edge with largest commodity and latest time window
                    ...
                    edges = sorted([((G.nodes[e[0]]['early'], len(self.commodities) - abs(k-e[0][0]) if is_node(e[0]) else 0), e[0]) for e in G.in_edges(v)], reverse=True)
    ...
    def solve(self, draw_filename='', start_time=time.time(), write_filename=''):
        ...
        # output statistics
        logger.info('{0:>3}, {1:>10}, {2:>10}, {3:>7}, {4:>6}, {5:>6}, {6}'.format(*'{0}#,LB,UB,Gap,Time,Solver,Type [TP]'.format('G' if self.model.solver == GUROBI else 'C').split(',')))
        ...
        while True:
            ...
            if USE_HEURISTIC_START:
                ...
                self.lower_bound = max(relaxed.objBound, self.lower_bound) if self.lower_bound != None else relaxed.objBound
                self.status = True if relaxed.status in [GRB.status.TIME_LIMIT, GRB.status.INTERRUPTED] and self.incumbent and (self.incumbent - self.lower_bound) < self.incumbent * self.GAP else (relaxed.status == gurobipy.GRB.status.OPTIMAL)
            else:
            ...
            if not self.status or solve_time > TIMEOUT:
                if self.model.is_abort() or solve_time > TIMEOUT:
                    ...
                    if not list(filter(lambda tw: tw[0] > tw[1], [solution.nodes[(k,c['b'][0])]['tw'] for k,c in enumerate(self.commodities)])):
                        t0 = time.time()
            ...
            if using_heuristic:
                ...
                if self.incumbent != None and best_obj > self.incumbent:
                    USE_HEURISTIC_START = False
            ...
            if draw_filename != '':
                ...
                #arcs = [G.edges(data=True) for G in self.timed_network]

                #pdf.draw_timeline_paper(self.intervals, arcs, 24 if not portrait else 15, self.get_solution())
            ...
            if path_failure and self.incumbent == None:
            ...
            if it_timepoints > 5: # arbitrary number
                ...
                for i,G_ in enumerate(nx.weakly_connected_components(solution)):
                    G = nx.subgraph(solution, G_)
            ...
            info.append((self.lower_bound, self.incumbent, time.time()-info_time, solve_time, len(tp - new_timepoints),  self.model.NumVars, self.model.NumConstrs, None, None, iterations, ((self.incumbent - self.lower_bound)/self.incumbent) if self.incumbent != None and self.incumbent > 0 else None))
            last_time = (time.time(), solve_time)
    ...
    def solve_heuristic_lower_bound(self, randomize):
        ...
        for k in K:
            ...
            for a1,a2,d in commodity_network.edges(data=True):
                # holding arcs
                if a1[0] == a2[0]:
                    d['weight'] = 0
                    continue
                ...
                # dispatch arcs
                if 'SOLUTION_K' not in consolidation_network[a1][a2]:
                    consolidation_network[a1][a2]['SOLUTION_K'] = set()

                remaining_quantity = sum([self.commodities[sk]['q'] for sk in consolidation_network[a1][a2]['SOLUTION_K']]) % self.network[a1[0]][a2[0]]['capacity']
                ...
                d['weight'] = self.problem.var_cost[k].get((a1[0],a2[0]),None) * self.commodities[k]['q'] + consolidation_cost + 0.001 # small constant to prevent cycles
            ...
            # find cheapest path
            path = nx.shortest_path(commodity_network, source=self.origin_destination[k][0], target=self.origin_destination[k][1], weight='weight')

            # update consolidation network solution
            for arc in pairwise(path):
                if arc[0][0] != arc[1][0]:
                    consolidation_network[arc[0]][arc[1]]['SOLUTION_K'].add(k)
        ...
        for a1,a2,d in consolidation_network.edges(data=True):
            ...
            consolidation_cost = self.network[a1[0]][a2[0]]['fixed_cost'] * ceil(quantity / self.network[a1[0]][a2[0]]['capacity'])
#            variable_cost = self.network[a1[0]][a2[0]]['var_cost'] * quantity
            variable_cost = sum([self.commodities[sk]['q'] * self.problem.var_cost[k].get((a1[0],a2[0]),0) for sk in d['SOLUTION_K']])
    ...
    def find_path_timepoint(self, k):
        ...
        return next(((n2, ek + self.restricted_shortest_path((dk,n2), (ok, n1)) + self.transit(n1,n2)) 
    ...
    def find_disjoint_timepoint(self, k1, k2, n1, n2):
        (ok1,ek1),(dk1,lk1) = self.commodities[k1]['a'],self.commodities[k1]['b']
        (ok2,ek2),(dk2,lk2) = self.commodities[k2]['a'],self.commodities[k2]['b']
        ...
        return (n2, tmp) if tmp + self.restricted_shortest_path((ok2, n1), (n2,dk2)) > lk2 else None


    ##
    ## adds timepoints that are proven (in paper) to find optimal solution in finite time (only using major theorems)
    ##
    def find_timepoints_default(self, solution, cycle):
        path_timepoints = set()
        ...
        for c in cycle:
            ...
            tw, start_node,i = sorted([(solution.nodes[n]['tw'], n, i) for i,n in enumerate(c) if is_node(n)])[0]
            t = tw[0]
            ...
            M = math.ceil((self.commodities[start_node[0]]['b'][1] - t) / cycle_length) * cycle_length + t
            ...
            for k in cycle_K:
                ...
                for p in pairwise(self.get_path(G, (k,origin[0]), enter)):
                    cycle_timepoints.add((p[1][1] if is_node(p[1]) else p[1][0][0], G.nodes[p[1]]['tw'][0]))

            for n in itertools.cycle(pairwise(list(map(operator.itemgetter(1), filter(is_node, c[i:] + c[:i]))) + [start_node[1]])):
        ...
        tails = [last for last in ((k,self.commodities[k]['b'][0]) for k in (K if path_failure == None else [path_failure])) 
                    if not cycle_timepoints and 'valid' in solution.nodes[last] and not solution.nodes[last]['valid']]
        ...
        return path_failure != None, path_timepoints, window_timepoints, cycle_timepoints, mutual_timepoints

    ##
    ## uses theorems from paper, preferring single timepoints.
    ##
    def find_timepoints_multiplex(self, solution, cycle):
        path_timepoints = set()
        window_timepoints = set()
        cycle_timepoints = set()
        mutual_timepoints = set()
        ...
        for G_ in nx.weakly_connected_components(solution):
            ...
            # Note: if cycle then 'valid' is not set and is ignored
            # get all commodities with invalid destination time (ignoring already processed items)
            tails = [last for last in ((k,self.commodities[k]['b'][0]) for k in K) 
                        if 'valid' in solution.nodes[last] and not solution.nodes[last]['valid']]
    ...
    ##
    def find_timepoints_all(self, solution, cycle):
        path_timepoints = set()
        window_timepoints = set()
        cycle_timepoints = set()
        mutual_timepoints = set()
        ...
        for G_ in nx.weakly_connected_components(solution):
            ...
            path_fail_K = set()  # keep track of commodities that fail path length

            for k in K:
                tw = solution.nodes[(k,self.commodities[k]['b'][0])]['tw']
            ...
            commodity_failed_consolidation = {k: next((n for n in self.get_path(G, (k, self.commodities[k]['a'][0]), (k,self.commodities[k]['b'][0]))
                                                         if not is_node(n) and solution.nodes[n]['tw'][1] < solution.nodes[n]['tw'][0]), None)
            ...
            for k,cons in commodity_failed_consolidation.items():
                if cons != None and not any(nx.has_path(G, n, cons) for n in first_failed_consolidation) and not any(nx.has_path(G, (i,self.commodities[i]['a'][0]), cons) for i in path_fail_K):
                    first_failed_consolidation = [cons] + [n for n in first_failed_consolidation if not nx.has_path(G, cons, n)]
            ...
            for cons in first_failed_consolidation:
                ...
                for k1,k2 in sorted(itertools.permutations(cons[1], 2), key=lambda x: abs(G.nodes[(x[0],n1)]['tw'][0]-G.nodes[(x[1],n1)]['tw'][1]), reverse=False):
            ...
            for k in sorted(K-path_fail_K, key=lambda k: self.commodities[k]['q'], reverse=True):
                ...
                if cons == None or any(nx.has_path(G, n, cons) for n in window):
                    continue

                n1,n2 = cons[0]

                for k2 in sorted(cons[1]-set([k]), key=lambda k2: abs(G.nodes[(k,n1)]['tw'][0]-G.nodes[(k2,n1)]['tw'][1]), reverse=True):
            ...
            for c in cycle:
                ...
                if  not (cycle_K & K) or any(nx.has_path(solution, n, c[0]) for n in window) or any(nx.has_path(solution, (k, self.commodities[k]['a'][0]), c[0]) for k in path_fail_K):
                    continue
                ...
                for cons in c:
                    ...
                    for k1,k2 in sorted(itertools.permutations(cons[1] & cycle_K, 2), key=lambda x: abs(G.nodes[(x[0],n1)]['tw'][0]-G.nodes[(x[1],n1)]['tw'][1]), reverse=False):
                ...
                # want to start at earliest point in cycle, and rewrite cycle to match
                tw, start_node,i = sorted([(solution.nodes[n]['tw'], n[1], i) for i,n in enumerate(c) if is_node(n)])[0]
                t = tw[0]
                ...
                for n in itertools.cycle(pairwise(list(map(operator.itemgetter(1), filter(is_node, c[i:] + c[:i]))) + [start_node])):
                    cycle_timepoints.add((n[0], t))
            ...
            # get all commodities with invalid destination time (ignoring already processed items)
            tails = [last for last in ((k,self.commodities[k]['b'][0]) for k in K) 
                        if 'valid' in solution.nodes[last] and not solution.nodes[last]['valid'] and 
                            not any(nx.has_path(solution, w, last) for w in window) and 
                            not any(nx.has_path(solution, (kp, self.commodities[kp]['a'][0]), last) for kp in path_fail_K)]
    ...
    def get_path(self, G, r,t):
        ...
        def prioritize_commodity(x):
            if is_node(x):
                if x[0] == t[0]:
                    ...
                    return G.nodes[n]['tw'][1] # earliest 'late time window'
            return 0
        ...
        while n != t:
            arcs = G.edges(n)
            n = sorted([a[1] for a in arcs if nx.has_path(G, a[1], t)], key=prioritize_commodity)[0]
            yield n
    
    ##
    ## Gets timepoints to break a generic acyclic solution
    ##
    def get_network_timepoints(self, solution, G, path, all_timepoints=False):
        r,t = path[0], path[-1]    
        ...
        def canDrop(v):
            ...
            return not all_timepoints and v != last and v != first and self.shortest_path(r[0], cr['a'][0], x) != None and cr['a'][1] + self.shortest_path(r[0], cr['a'][0], x) >= solution.nodes[v]['early']
        ...
        for n,n2 in pairwise(itertools.dropwhile(canDrop, path)):
            ...
            if solution.nodes[n]['early'] > self.T:
                break

            has_k,x = (n[0] == t[0], n[1]) if is_node(n) else (t[0] in n[1], n[0][0])
            tp.add((x, solution.nodes[n]['early']))
            ...
            x2_early = solution.nodes[t[0],x2]['early'] if (t[0],x2) in solution else None

            if not all_timepoints and has_k and x2_to_dest != None and x2_early != None and x2_early + x2_to_dest > ct['b'][1]:
                if next_tk == n and solution.nodes[n2]['early'] < self.T:
                    tp.add((x2, solution.nodes[n2]['early']))

                return tp

        if solution.nodes[t]['early'] < self.T:
            tp.add((t[1], solution.nodes[t]['early']))
    ...
    def printSolution(self, incumbent=True):
        sol = self.incumbent_solution if incumbent else (self.solution_paths, sorted([((c[0][0],c[1][0]), frozenset(k)) for c,k in self.consolidations.items() if len(k) > 1]), self.get_in_tree_paths())
    ...
    def get_statistics(self):
        stats = {}
        stats['cost'] = self.model.objVal() if self.status and self.incumbent == None else self.incumbent
    ...
    def get_solution(self):
        return tuplelist((k,(a1,a2)) for k,G in enumerate(self.timed_network) for a1,a2,d in G.edges(data=True) if 'x' in d and self.model.val(d['x']) > 0) if self.status else None
    ...
    def get_solution_arcs(self):
        result = {}
        arcs = dict(((k,a[0]),a[1]) for (k,a) in self.get_solution())
    ...
    def get_network_solution(self, paths=None, cons=None):
        ...
        if cons == None:
            cons = [(c, frozenset(k)) for c,K in self.consolidations.items() for k in K if len(k) > 1]
        ...
        solution.add_nodes_from((k,n) for k,P in enumerate(paths) for n in P.nodes())
        ...
        for n in solution:
            if not is_node(n):
                solution.nodes[n]['tw'] = (max(tw[k].nodes[n[0][0]]['early'] for k in n[1]), min(tw[k].nodes[n[0][0]]['late'] for k in n[1]))
            else:
                solution.nodes[n]['tw'] = (tw[n[0]].nodes[n[1]]['early'], tw[n[0]].nodes[n[1]]['late'])# tw[n[0]][n[1]]
        ...
        for G_ in nx.weakly_connected_components(solution):
            ...
            for n in nx.dfs_postorder_nodes(G):
                ...
                if len(coll) > 1:
                    # check for cycles
                    p = next((p[1] for p in coll if 'late' not in solution.nodes[p[1]]), None)
                    if p:
                    ...
                    solution.nodes[n]['late'] = round(min(solution.nodes[p[1]]['late'] - (self.transit(n[1], p[1][1]) if is_node(n) and is_node(p[1]) else 0) for p in coll) - (self.transit(*n[0]) if not is_node(n) else 0), PRECISION)
                elif len(coll) == 1:
                    p = coll[0][1]

                    # check for cycles
                    if 'late' not in solution.nodes[p]:
                        cycle.append(nx.shortest_path(solution, p, n))
                        has_cycle = True
                        break 

                    solution.nodes[n]['late'] = round(solution.nodes[p]['late'] - (self.transit(n[1], p[1]) if is_node(p) else 0), PRECISION)
                else: # at end
                    solution.nodes[n]['late'] = self.commodities[n[0]]['b'][1]
            ...
            if not has_cycle:
                ...
                for n in nx.dfs_postorder_nodes(G.reverse()):
                    ...
                    if len(coll) > 1:
                        ...
                        early = max(solution.nodes[p[0]]['early'] + (self.transit((p[0][1] if is_node(p[0]) else p[0][0][0]), n[1]) if is_node(n) else 0) for p in coll)
                        min_early = min(solution.nodes[p[0]]['early'] + (self.transit((p[0][1] if is_node(p[0]) else p[0][0][0]), n[1]) if is_node(n) else 0) for p in coll)

                        solution.nodes[n]['early'] = round(early, PRECISION)
                        solution.nodes[n]['diff'] = round(early - min_early, PRECISION)
                    elif len(coll) == 1:
                        p = coll[0][0]
                        solution.nodes[n]['early'] = round(solution.nodes[p]['early'] + self.transit(p[1] if is_node(p) else p[0][0], n[1]), PRECISION)
                    else: # at end
                        solution.nodes[n]['early'] = self.commodities[n[0]]['a'][1]

                    solution.nodes[n]['valid'] = solution.nodes[n]['early'] <= solution.nodes[n]['late']
    ...
    def create_shortest_paths(self, opt=shortest_path_option.shared):
        self.shared_shortest_paths = {a[0]: a[1] for a in nx.shortest_path_length(self.network, weight='weight')} # nx.shortest_path_length(self.network, weight='weight')
                ...
                for j in self.network:
                    if i <= j:
                        ...
                        for n2 in self.network:
                            if i != n2 and j != n2:
                                self.edge_shortest_path[((i,j),n2)] = nx.shortest_path_length(network_copy, n2, weight='weight')

    # gets the tightened shortest path
    def restricted_shortest_path(self, reject_nodes, arc):
    ...
    def V(self, k, a):
        ...
        if a[0][0] != a[1][0]:
            origin_to_arc = self.restricted_shortest_path((dest[0], a[1][0]), (origin[0], a[0][0])) 
            arc_to_dest = self.restricted_shortest_path((origin[0], a[0][0]), (a[1][0], dest[0])) 
    ...
    def add_network_timepoints(self, new_timepoints):
        ...
        for n,t in new_timepoints:
            ...
            self.cons_network.add_node(i1, attr_dict=self.cons_network.nodes[i0])
            self.cons_network.add_node(i2)

            i1_out_edges = [(i1, target, data) for (_,target,data) in self.cons_network.edges(i0, data=True)]
            i1_in_edges = [(source, i1, data) for (source,_,data) in self.cons_network.in_edges(i0, data=True)]
        ...
        if self.ALGORITHM >= algorithm_option.reduced:
            for n in self.cons_network:
                for key,g in itertools.groupby(sorted(self.cons_network.edges(n, data=True)), lambda t: t[1][0]):
        ...
        for a1,a2,d in self.cons_network.edges(data=True):
            if 'z' not in d:
                d['z'] = self.model.addVar(obj=(self.network[a1[0]][a2[0]]['fixed_cost']), lb=0, 
                                           #ub=ceil(sum(self.commodities[c]['q'] for c in d['K'])),
                                           ub=self.model.inf(), 
        ...
        for k,G in enumerate(self.timed_network):
            for a1,a2,d in G.edges(data=True):
                if 'x' not in d and (a1[0] == a2[0] or self.cons_network[a1][a2]['z'] != None):
                    d['x'] = self.model.addVar(obj=(self.problem.var_cost[k].get((a1[0],a2[0]),0) * self.commodities[k]['q'] if a1[0] != a2[0] else self.problem.var_cost[k].get((a1[0],a2[0]),0) * self.commodities[k]['q'] * (a1[2] - a1[1])), lb=0, ub=1, type=self.model.binary() if ALLOW_SPLIT == False else self.model.continuous(), name='x' + str(k) + ',' + str((a1,a2)))
                    ...
                    if a1[0] == a2[0] and a1[2] > a1[1] + 1:
                        d['y'] = self.model.addVar(obj=(-self.problem.var_cost[k].get((a1[0],a2[0]),0) * self.commodities[k]['q'] * (a1[2] - a1[1])), lb=0, ub=1, type=self.model.binary() if ALLOW_SPLIT == False else self.model.continuous(), name='y' + str(k) + ',' + str((a1,a2)))
    ...
    def add_network_timepoint(self, k, n, i0, i1, i2):
        ...
        # Relabel nodes (i0 -> i1)
        G.add_node(i1, attr_dict=G.nodes[i0])
        i1_out_edges = [(i1, target, data) for (_,target,data) in G.edges(i0, data=True)]
        i1_in_edges = [(source, i1, data) for (source,_,data) in G.in_edges(i0, data=True)]
        G.remove_node(i0)

        # Copy appropriate edges from i1 (out / in)
        i2_edges = list(filter(v, ((i2,e2) for e1,e2,d in i1_out_edges))) + list(filter(v, ((e1,i2) for e1,e2,d in i1_in_edges)))
        new_edges.extend(i2_edges)

        # Remove invalid edges from i1, Keep good edges
        del_edges, keep_edges = partition(lambda t: v((t[0],t[1])), i1_out_edges + i1_in_edges)

        new_edges.extend(keep_edges)
        G.add_edges_from(new_edges)

        ## Update consolidation network
        for a1,a2 in i2_edges:
            if a1[0] != a2[0]:
        ...
        # rename arcs
        for a,a0,d in itertools.chain((((a1,a2),(i0,a2),d) for a1,a2,d in G.out_edges(i1, data=True)), (((a1,a2),(a1,i0),d) for a1,a2,d in G.in_edges(i1, data=True))):
            if 'x' in d:
            ...
            for a,x in arcs.items():
                ...
                if a[0][0] != a[1][0]:
                    ...
                    if a not in self.constraint_consolidation:
                        self.constraint_consolidation[a] = self.model.addConstr(x * self.commodities[k]['q'] <= self.cons_network[a[0]][a[1]]['z'] * self.network[a[0][0]][a[1][0]]['capacity'], 'cons' + str(a))
                    else:
                    ...
                    if self.constraint_cycle != None:
                        if (k,a[0][0]) not in self.constraint_cycle:
                            self.constraint_cycle[(k,a[0][0])] = model.addConstr(x <= 1, 'cycle')
                        else:
                    ...
                    # user_cuts
                    if 'z' in self.cons_network[a[0]][a[1]] and self.cons_network[a[0]][a[1]]['z'] != None:
                        if USER_CUTS:
                            self.constraints_user.append(self.model.addConstr(self.cons_network[a[0]][a[1]]['z'] >= math.ceil(self.commodities[k]['q']/self.network[a[0][0]][a[1][0]]['capacity']) * x, 'user'))

                        if ORIGIN_CUTS:
                            if a[0][0] == self.commodities[k]['a'][0]:
                                chg_coeff.append((self.constraints_origin[k], self.cons_network[a[0]][a[1]]['z'], 1))

                            if a[1][0] == self.commodities[k]['b'][0]:
                                chg_coeff.append((self.constraints_dest[k], self.cons_network[a[0]][a[1]]['z'], 1))
                ...
                if (k,a[0]) not in self.constraints_holding_offset:
                    i = [d['x'] for a1,a2,d in self.timed_network[k].in_edges(a[0], data=True) if 'x' in d and a1[0] != a2[0]]
                    o = [(d['x'], d['y']) for a1,a2,d in self.timed_network[k].out_edges(a[0], data=True) if 'x' in d  and 'y' in d and a1[0] == a2[0]] # get holding arc out
